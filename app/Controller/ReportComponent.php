...
*/
	...
	protected function __AddEmptyDataFields($data){
	  if($this->_controller->request->projectvars['evalId'] > 0) return $data;
		...
		$evaluationSchema = $this->_controller->$ReportEvaluation->schema();

		foreach($evaluationSchema as $key => $value) {
			$data[$ReportEvaluation][$key] = '';
		}
	...
	public function DevelopmentsEnabled($data) {
		...
		if(is_countable($Data['Welds']) && count($Data['Welds']) == 0) return $data;
	...
	public function GetRepairReport($reportnumber) {
		...
		foreach($reportnumber[$ReportEvaluation] as $_key => $_data){
			...
			$optionsRepair = array('conditions' => array('Repair.evaluation_id_mistake' => $_data[$ReportEvaluation]['id']));
...
*/
		...
		if(!isset($data[$ReportEvaluation]['position'])){
			...
			if(isset($paginationOverview['weld_struktur_id'])) {
	...
	protected function __CheckForMasterDropdown($xml){
		...
		//Prüfung um undefined index zu umgehen
		if(isset($Areas[$Area[$id]])) {
			if(empty($Areas[$Area[$id]])) return false;
		} else {
			return false;
		}
...
//		if(array_search(strtolower(trim($xml->select->custom_field)), array('1','yes')) !== false) return null;
		...
		$Check = $this->__CheckForMasterDropdown($xml);
