 ...
 */
class TestingmethodsController extends AppController {
...
//		$this->set('breadcrumbs', $this->Navigation->Breadcrumb());
		...
		$this->writeprotection = 0 < $this->Testingmethod->Report->Reportlock->find('count', array('conditions'=>array('Reportlock.topproject_id'=>$this->request->projectID, 'Reportlock.report_id'=>$this->request->reportID)));
...
//			$SettingsArray['settingslink'] = array('discription' => __('Settings',true), 'controller' => 'topprojects','action' => 'settings', 'terms' => null,);
	...
	public function listing() {
		...
		$reportsArray = $this->Navigation->SubMenue('Report','Topproject',__('Create', true),'listing','testingmethods',$projectID);
		...
		if($this->request->reportnumberID > 0 && isset($this->request->data['linked']) && $this->request->data['linked'] == 1) {
			...
			$SettingsArray['backlink'] = array('discription' => __('Back',true), 'controller' => 'reportnumbers','action' => 'assignedReports', 'terms' => $this->request->projectvars['VarsArray']);

			if(isset($this->request->data['setGenerally'])){
				$this->set('setGenerally',$this->request->data['setGenerally']);
			}
		...
		if(count($this->Testingmethod->find('all', $options)) == 0) {
		...
		$this->Testingmethod->recursive = 0;
		$testingmethods = $this->Testingmethod->find('all', $options);
 ...
 */
	public function view($id = null) {
		if (!$this->Testingmethod->exists($id)) {
		...
		$options =
			array(
				'conditions' => array(
					'Testingmethod.'.$this->Testingmethod->primaryKey => $id
				),
				'contain' => array(
					'Qualification' => array(
						'conditions' => array(
		...
		$result = $this->Testingmethod->find('first', $options);

		$this->set('breads', $this->Navigation->Breads(null));
		$this->set('testingmethod', $this->Testingmethod->find('first', $options));
	}
 ...
 */
	public function add() {
		if (isset($this->request->data['Testingmethod'])) {
			$this->Testingmethod->create();
			$this->Autorisierung->ConditionsTesting($this->request->data['Topproject']['Topproject'],'conditionsTopprojects');
			if ($this->Testingmethod->save($this->request->data)) {
		...
		$topprojects = $this->Testingmethod->find('list',$OptionsTopprojects);
		$this->set('breads', $this->Navigation->Breads(null));
 ...
 */
	public function edit($id = null) {
		...
		if (isset($this->request->data['Testingmethod']) || $this->request->is('put')) {
			$this->Autorisierung->ConditionsTesting($this->request->data['Topproject']['Topproject'],'conditionsTopprojects');
			if ($this->Testingmethod->save($this->request->data)) {
				$this->Session->setFlash(__('Testingmethod was saved'));
				$this->set('afterEDIT', $this->Navigation->afterEDIT($this->request->params['controller'],5000));
		...
		$OptionsTestingmethod =
			...
			$this->request->data = $this->Testingmethod->find('first', $OptionsTestingmethod);
		...
		$OptionsTopprojectsSelect = array('conditions' => array('Topprojects.id' => $this->Autorisierung->ConditionsTopprojects()));
		$OptionsTopprojects = array('conditions' => array('id' => $this->Autorisierung->ConditionsTopprojects()));
		$topprojects = $this->Testingmethod->Topproject->find('list', $OptionsTopprojects);
		$this->set('breads', $this->Navigation->Breads(null));
		$this->set(compact('topprojects'));
		$this->set('topprojects_select', $this->SelectValue->SelectData('Topprojects','projektname',$OptionsTopprojectsSelect));
	}
 ...
 */
	public function delete($id = null) {
		$this->Testingmethod->id = $id;

		$options =
			array(
				'conditions' => array(
					'Testingmethod.'.$this->Testingmethod->primaryKey => $this->Testingmethod->id
				),
				'contain' => array(
					'Qualification' => array(
						'conditions' => array(
		...
		$_testingmethod = $this->Testingmethod->find('first', $options);
		...
		$this->Autorisierung->ConditionsTesting($_topprojectIdArray,'conditionsTopprojects');
		if (!$this->Testingmethod->exists()) {
			throw new NotFoundException(__('Invalid testingmethod'));
		}
		$this->request->onlyAllow('post', 'delete');
		if ($this->Testingmethod->delete()) {
			$this->Session->setFlash(__('Testingmethod was deleted'));
			$this->set('afterEDIT', $this->Navigation->afterEDIT($this->request->params['controller'],5000));
		}
		$this->Session->setFlash(__('Testingmethod was not deleted'));
