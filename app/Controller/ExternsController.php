...
class ExternsController extends AppController {
...
//			$this->Auth->logout();
	...
	public function index() {
		$this->redirect(array('controller' => 'externs', 'action' => 'externreports'));
	}
...
//					$decrypted_id = $this->Autorisierung->DecriptIncommingId($UserInfoSave['reportnumber_id']);
					$decrypted_id = $this->Sicherheit->SoduimDecrypt($UserInfoSave['reportnumber_id']);
...
//						$ticket_id = bin2hex(Security::rijndael(($this->Extern->ExternData->getLastInsertId()), Configure::read('SignatoryHash'), 'encrypt'));
						$ticket_id = $this->Sicherheit->SoduimEncrypt($this->Extern->ExternData->getLastInsertId());
					...
					$this->redirect(array('controller' => 'externs', 'action' => 'incomming', $id));
				}
			return;
			}
...
//		$decrypted_id = $this->Autorisierung->DecriptIncommingId($id);
		$decrypted_id = $this->Sicherheit->SoduimDecrypt($id);
		// User ID 
		...
		$IpInfos = array();
		$IpInfo = $this->Autorisierung->IpInfo($ClientIp,'Location');
		if(count($IpInfo) > 0) $IpInfos = $IpInfo;	
	
		// Alle Infos über User holen und in ein Array schreiben
		$UserInfo = array_merge(array('ticket' =>$Ticket),array('reportnumber_id' =>$id),array('ip' =>$ClientIp),array(),$IpInfos);
	...
	public function externreports() {
		...
		if($this->Session->read('UserInfo.ticket_id') != $this->Cookie->read('ticket')) $this->redirect(array('controller' => 'externs', 'action' => 'incomming'));
/*
...
*/
		$ticket_id = $this->Sicherheit->SoduimDecrypt($this->Session->read('UserInfo.ticket_id'));
		$reportnumber_id = $this->Sicherheit->SoduimDecrypt($this->Session->read('UserInfo.reportnumber_id'));
		$ticket = $this->Sicherheit->SoduimDecrypt($this->Cookie->read('ticket'));
		
		if(!is_int($ticket_id)) $this->redirect(array('controller' => 'externs', 'action' => 'incomming'));
		if(!is_int($reportnumber_id)) $this->redirect(array('controller' => 'externs', 'action' => 'incomming'));
		if(!is_int($ticket)) $this->redirect(array('controller' => 'externs', 'action' => 'incomming'));

		if($ticket_id != $ticket) $this->redirect(array('controller' => 'externs', 'action' => 'incomming'));
		...
		$this->Reportnumber->recursive = -1;
		$report_number_test = $this->Reportnumber->find('first',array('fields' => array('Reportnumber.id'), 'order' => array('Reportnumber.id desc')));
		if($report_number_test['Reportnumber']['id'] < $reportnumber_id)  $this->redirect(array('controller' => 'externs', 'action' => 'incomming'));
		...
		foreach($ReportKey as $_key => $_ReportKey){
			...
			if($xml['settings']->$_ReportKey->children() == null) continue;
		...
		if(NULL !== $xml['settings']->$ReportKey['Evaluations'] && !empty($xml['settings']->$ReportKey['Evaluations']->children()) && count($Reportobjekt->$ReportKey['Evaluations']) > 0){
