 ...
 */
	public $columns = array(
		...
		'datetime' => array('name' => 'datetime', 'format' => 'Y-m-d H:i:s', 'formatter' => 'date', 'srcformatter'=>'strtotime'),
		'timestamp' => array('name' => 'timestamp', 'format' => 'Y-m-d H:i:s', 'formatter' => 'date', 'srcformatter'=>'strtotime'),
		'time' => array('name' => 'time', 'format' => 'H:i:s', 'formatter' => 'date', 'srcformatter'=>'strtotime'),
		'date' => array('name' => 'date', 'format' => 'Y-m-d', 'formatter' => 'date', 'srcformatter'=>'strtotime'),
...
//		$this->log($sql, 'SQL');
	...
	protected function createNumericFunctions() {
	return;
		// https://venerableagents.wordpress.com/2011/01/29/mysql-numeric-functions/
		$this->_connection->exec('DROP FUNCTION IF EXISTS IsNumeric');
		$this->_connection->exec('CREATE FUNCTION IsNumeric (val varchar(255)) RETURNS tinyint RETURN val REGEXP \'^(-|\\\\+){0,1}([0-9]+\\\\.[0-9]*|[0-9]*\\\\.[0-9]+|[0-9]+)$\'');
		$this->_connection->exec('DROP FUNCTION IF EXISTS NumericOnly');
		$this->_connection->exec('CREATE FUNCTION NumericOnly (val VARCHAR(255)) RETURNS VARCHAR(255)
			BEGIN
