<?php
  ...
  // App::uses('Component', 'Controller/Component');
   //     App::uses('QualificationComponent', 'Controller/Component');
 //$Qualification = new QualificationComponent();

     //   $this->Examiner->recursive = 1;
     //   $examiner = $this->Examiner->find('all');
		...
		foreach($certificate as $_key => $_certificates){
                    ...
                    if(isset($_certificates ['CertificateData'])) {
        ...
        if (Configure::check('monitoring.email.to')&& Configure::read('monitoring.email.to') <> "") {
          $mails =  Configure::read('monitoring.email.to');
          $email->to($mails);
        ...
        $email->subject('Zertifizierungsinfos');
		...
		foreach($certificate as $_key => $_certificate){
			...
			if(count($data) > 0){
				...
				foreach($data as $__key => $__data){
					...
					$certificates[$_certificate][$__key] = $this->TimeHorizons($certificates[$_certificate][$__key],array('main'=>'Certificate','sub'=>'CertificateData'),$certificates[$_certificate][$__key]['CertificateData']['period']);
	...
	public function IsRenewal($certificate) {
		// Test ob Erneuert oder rezertifiziert wird
		if($certificate['CertificateData']['renewal_in_year'] > 0){

			$count = $this->Examiner->Certificate->CertificateData->find('count',array(
					'conditions' => array(
						'CertificateData.certificate_id' => $certificate['CertificateData']['certificate_id'],
						'CertificateData.deleted' => 0
						)
					)
				);
			...
			if($rest == 0){
				$certificate['CertificateData']['period'] = $certificate['Certificate']['recertification_in_year'] - $certificate['Certificate']['renewal_in_year'];
				$certificate['CertificateData']['renewal'] = false;
			}
			else {
				$certificate['CertificateData']['period'] = $certificate['Certificate']['renewal_in_year'];
				$certificate['CertificateData']['renewal'] = true;
			}
		}
		else {
			$certificate['CertificateData']['period'] = $certificate['Certificate']['recertification_in_year'];
		}
		return $certificate;
	}
...
public function TimeHorizons($_data,$model /*,$period*/) {
       ...
       $_data[$model['sub']]['valid'] = "";
			...
			//$nextCertification = date('Y-m-d',strtotime('+' . $_data[$model['sub']]['period'] . ' year', strtotime($_data[$model['sub']]['certified_date'])));
...
__('The time for certification is reached on',true). ' ' . $___data[$model['sub']]['next_certification'].' (' . __('in',true) . ' ' . $this->dateDiff($___data[$model['sub']]['next_certification'],date('Y-m-d',time())).')';
				...
				if($___data[$model['main']]['certificate_data_active'] == 1 && $___data[$model['sub']]['valid'] == 0 && isset($___data[$model['sub']]['active']) && $___data[$model['sub']]['active'] == 1){
				...
				if($___data[$model['main']]['certificate_data_active'] == 1 && $___data[$model['sub']]['certified_file'] == '' && isset($___data[$model['sub']]['active'] ) && $___data[$model['sub']]['active'] == 1){
      ...
      foreach ($MonitoringKind as $mkey => $mvalue) {
        ...
        foreach($certificate as $_key => $_certificates){
          ...
          if($emptysummary == 0) {
            ...
            $Options = array(
              'fields' => array('DISTINCT working_place','email'),
            );

            $EmailsCronjob = $this->ExaminersEmailCronjob->find('all',$Options);

            if(count($EmailsCronjob) == 0) {
              return;
            }

            $emails = Hash::extract($EmailsCronjob, '{n}.ExaminersEmailCronjob.email');
            ...
            foreach ($emails as $ekey => $evalue) {
              ...
              $email->from('info@mbq.docu-dynamics.cloud');

              $email->to($evalue);
              !empty($mailed)?$betreff = 'Geraeteueberwachung'.' '.$mvalue['DeviceCertificate']['certificat'].' noch offen: '.$mailed:$betreff = 'Geraeteueberwachung'.' '.$mvalue['DeviceCertificate']['certificat'];
/*
			  print '<pre>';
			  print_r($summary);
			  print '</pre>';
