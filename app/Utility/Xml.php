 ...
 */
	protected static function _loadXml($input, $options) {
		if(preg_match('/encoding="utf[-]?+8"/i', $input))
		{
			$db = ConnectionManager::getDatasource('default');
			if(method_exists($db, 'setUTF8Encoding'))
			{
				$input = $db->setUTF8Encoding($input);
			} 
			else
			{
//				CakeLog::error('Function setUTF8Encoding does not exist in '.$db);
			}
		}
		...
		try {
			...
			{
				CakeLog::error($error->message.PHP_EOL.'line '.$error->line.': '.$xml[$error->line-1].PHP_EOL.'column '.$error->column);
			}
			$xml = null;
		}
		if ($hasDisable && !$options['loadEntities']) {
