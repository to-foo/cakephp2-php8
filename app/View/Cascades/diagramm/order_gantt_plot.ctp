...
$graph->scale->actinfo->SetColTitles( array(__('Equipments',true),__('Start',true),__('End',true),__('Interval',true),__('Rest time',true),__('Advance',true)),array(100));
...
foreach ($data['Status'] as $key => $value) {
  ...
  $bar = new GanttBar($y,array(
        ...
        $data['Scheme']['AdvancesOrder']['Date']['interval'],
        $data['Scheme']['AdvancesOrder']['Date']['resttime'],
        strval($value['result']['advance_all_percent'])
			),
		$data['Scheme']['AdvancesOrder']['Date']['start'],
		$data['Scheme']['AdvancesOrder']['Date']['end'],
		0.35
