...
if(!isset($data['Scheme']['AdvancesOrder'])) return null;
if(!isset($data['Scheme']['AdvancesOrder']['Date'])) return null;
...
$graph->scale->actinfo->SetColTitles( array(__('Methode',true),__('Start',true),__('End',true),'   ',__('Count',true),__('Completed',true),__('repair',true)),array(100));
...
foreach ($data['Status'] as $key => $value) {
  ...
  if(!isset($data['Scheme'])) continue;
  if(!isset($value['result'])) continue;
  if(!isset($value['advance_line_color'])) continue;
  if($key == 'all_advance_methods') continue;

  $bar = new GanttBar($y,array(
				__($key,true),
				$data['Scheme']['AdvancesOrder']['Date']['start'],
				$data['Scheme']['AdvancesOrder']['Date']['end'],
        '   ',
        strval($value['result']['count']),
        strval($value['result']['okay']),
        strval($value['result']['error'])
			),
		$data['Scheme']['AdvancesOrder']['Date']['start'],
		$data['Scheme']['AdvancesOrder']['Date']['end'],
		0.35
