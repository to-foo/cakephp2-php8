...
foreach($welder['WelderTime'] as $time) {
	$tcpdf->SetY($tcpdf->GetY()+$define['QM_CELL_PADDING_T']);
	$testing_start = strtotime($time['WelderTime']['testing_time_start']);
	$testing_end = strtotime($time['WelderTime']['testing_time_end']);
	
	$waiting_start = isset($time['WelderTime']['waiting_time_start']) ? strtotime($time['WelderTime']['waiting_time_start']) : null;
	$waiting_end = isset($time['WelderTime']['waiting_time_end']) ? strtotime($time['WelderTime']['waiting_time_end']) : null;
	...
	$tcpdf->MultiCell($cols[7], $height, trim($time['WelderTime']['remarks']), $border, 'C', true, 0, $tcpdf->GetX()+$define['QM_CELL_PADDING_L'], $tcpdf->GetY(), true, 0, false, true, $height, 'M', true);	
...
$tcpdf->Output(str_replace(' ', '_', __('Welder workload %s', $welder['Welder']['name'])).'.pdf', 'D');