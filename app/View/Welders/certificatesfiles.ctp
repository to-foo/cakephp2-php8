...
ucfirst($_data[$models['main']]['weldingmethod']) . ' - ' .
__('documents',true)
;
?>
</h2>
		...
		<div class="files">
			<table cellpadding="0" cellspacing="0">
				...
				foreach ($_files as $__files):
					if(isset($__files['WelderCertificatefile']['error'])) continue;
				?>
				<tr>
					<td>
		...
		$("span.for_hasmenu1").contextmenu({
			...
			menu: [
				...
				action :	function(event, ui) {
							$("#dialog").load("welders/certificatefilesdescription/" + ui.target.attr("rev"), {
									"ajax_true": 1
								});
							$("#dialog").dialog("open");
							},
				...
				cmd: "status",
				action :	function(event, ui) {
							$("#dialog").load("welders/delcertificatefiles/" + ui.target.attr("rev"), {
									"ajax_true": 1
								});
							$("#dialog").dialog("open");
							},
