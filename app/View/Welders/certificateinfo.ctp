...
echo '<div class="hint certification ' .  $certificate_data['WelderCertificateData']['valid_class'] . '">';
if($certificate_data['WelderCertificateData']['active'] == 0){
	echo '<p><strong>';
	echo __('This Qualification was deactivated.',true); 
	echo '</strong></p>';
}
echo '<p>';
echo __('Zertifiziert am',true) . ': ' . $certificate_data['WelderCertificateData']['certified_date']; 
echo '</p>';
if($certificate_data['WelderCertificateData']['certified'] == 1){
	echo '<p>';
	echo $certificate_data['WelderCertificateData']['time_to_next_certification']; 
	echo '</p><p>';
	echo $certificate_data['WelderCertificateData']['time_to_next_horizon'] . ' ';
	echo '</p>';
}
if($certificate_data['WelderCertificateData']['certified'] == 2){
	echo '<p>';
	echo __('This certificate has the status',true) . ' "' . __('not certificated',true) . '"';
	echo '</p>';
}
if($certificate_data['WelderCertificateData']['certified'] == 0){
	echo '<p>';
...
echo '<p>';
echo '<p>' . $certificate_data['WelderCertificateData']['next_certification_date'] . '</p>';
echo '<p><b class="';
echo $certificate_data['WelderCertificateData']['certification_requested_class'];
echo '">' . $certificate_data['WelderCertificateData']['certification_requested'] . '</b></p>';
echo '<p>';
if($certificate_data['WelderCertificateData']['certification_requested_class'] == 'is_not_requested' && $certificate_data['WelderCertificateData']['valid'] != 0){
...
//if($certificate_data['WelderCertificateData']['valid'] != 0){
	if($certificate_data['WelderCertificateData']['apply_for_recertification'] == 1){
...
if($certificate_data['WelderCertificateData']['apply_for_recertification'] == 1){
	...
	if(isset($certificate_data['WelderCertificateData']['renewal']) && $certificate_data['WelderCertificateData']['renewal'] == true){
...
/*
print '<pre>';
print_r($certificate_data['WelderCertificateData']);
print '</pre>';
*/
echo '</div>';
