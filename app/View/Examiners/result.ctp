...
<div>
<?php
 //   echo $this->Html->link(__('Print examiner list',true), array_merge(array('action' => 'pdfinv'), $this->request->projectvars['VarsArray']), array('class' => 'icon icon_examiners_pdf','title' => __('Print examiner list',true)));?>
</div>
<div>
<?php
if(($this->Session->check('searchexaminer.params'))) {
	echo '<div class="hint"><p>';
        echo $this->Session->read('searchexaminer.params');
...
<table class="table_resizable table_infinite_sroll">
	...
	<?php
		$i = 0;
		if (isset($examiners))foreach ($examiners as $examiner):
		...
		if($examiner['Examiner']['active'] == 0){
			$class = ' class="infinite_sroll_item deactive" title="'.__('This examiner is deactive',true).'"';
		}

		$this->request->projectvars['VarsArray'][13] = 0;
		$this->request->projectvars['VarsArray'][14] = 0;
		$this->request->projectvars['VarsArray'][16] = $examiner['Examiner']['id'];


		echo '<tr '.$class.'>';
                echo '<td class="small_cell">';
		echo'<span class="for_hasmenu1 weldhead">';
		 echo $this->Html->link($examiner['Examiner']['name'] . ' ' . $examiner['Examiner']['first_name'],
...
//			if(trim($_xml->condition->key) != 'enabled') continue;
			...
			$this->request->projectvars['VarsArray'][15] = isset($examiner['ExaminerTestingmethod'][0]['id']) ?$examiner['ExaminerTestingmethod'][0]['id'] : '';
		...
		if(isset($summary[$examiner['Examiner']['id']]['summary']['qualifications'])){
			...
			foreach($summary[$examiner['Examiner']['id']]['summary']['qualifications'] as $_key => $_qualifications){
				...
				$thissummary = $this->Quality->CertificatSummarySingle($summary[$examiner['Examiner']['id']],$_qualifications['certificate_id'],$_qualifications['certificate_data_id']);

				echo '<div class="container_summary_single container_summary_single_'.$_key.'_' . $examiner['Examiner']['id'] . '">';
				echo $thissummary;
				echo '</div>';
				...
				echo $this->Html->link($_key,array_merge(array('action' => 'qualifications'),$this_link),array('title' => $_key,'rev'=> $_key,'rel'=> $examiner['Examiner']['id'], 'class' => 'summary_tooltip ajax icon '.$_qualifications['class']));
			}
		}
		echo '</span></td>';
		...
		if(isset($summary[$examiner['Examiner']['id']]['eyecheck']['qualifications']) && count($summary[$examiner['Examiner']['id']]['eyecheck']['qualifications']) > 0){
			...
			//$this_link = $this->request->projectvars['VarsArray'];
			echo '<a href="examiners/eyechecks/'.$_eyecheck['termlink'].'" title="'.$_eyecheck['certificat'].'" rel="'.$_eyecheck['certificate_id'].'" class="summaryeyecheck_tooltip ajax icon '.$_eyecheck['class'].'">'.$_eyecheck['certificat'].'</a>';
