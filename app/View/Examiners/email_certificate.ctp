...
<h2><?php echo __('Send this certificate informations per email');?></h2>
	...
	if(count($_summary) > 0 && isset($_key)){
		...
		foreach($_summary as $__key => $__summary){
			...
			foreach($__summary as $___key => $___summary){
				...
				echo $___summary['examiner']['name'].' '.$___summary['examiner']['working_place'].' - ';
				echo $___summary['certificate']['third_part'].'/'.$___summary['certificate']['sector'].'/'.$___summary['certificate']['testingmethod'].'/'.$___summary['certificate']['certificat'].'-'.$___summary['certificate']['level'].'<br>';
				echo '</p>';
				echo '<ul>';
