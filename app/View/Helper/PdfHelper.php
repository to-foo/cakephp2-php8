...
class PdfHelper extends AppHelper
{
    ...
    public function MakeHaederData($tcpdf, $position)
    {
        ...
        foreach ($tcpdf->xsettings->$Reports as $_Report) {
            foreach ($_Report as $__Report) {
                ...
                if (!empty($__Report->pdf->description->p) && $__Report->pdf->description->p != '') {
    ...
    public function MakeBodyData($tcpdf)
    {
        ...
        $evalDataX = ($tcpdf->xdata->xpath($Reports[2]));
        ...
        foreach ($Reports as $_Reports) {
            foreach ($tcpdf->xsettings->$_Reports as $_xsettings) {
                if ($_Reports == $Reports[2]) {
                    for ($id = 0; $id < $tcpdf->xdata->$_Reports->count(); ++$id) {
                        ...
                        foreach ($_xsettings as $__xsettings) {
                            if ($__xsettings->pdf->output == 4) {
                                ...
                                if (trim($evalData->$key) == '') {
                                    continue;
                                }

                                if (trim($evalData->$key) == '' && empty($__xsettings->pdf->headline)) {
                                    continue;
                                }
                                ...
                                if ($evalData->$key != '') {
                                    ...
                                    $footerData['data_array'] = $evalData->$key;
                                }
                                ...
                                if(!empty($__xsettings->pdf->fontcolordata)){
                                  $footerData['fontcolor'][0] = intval(trim($__xsettings->pdf->fontcolordata->color[0]));
                                  $footerData['fontcolor'][1] = intval(trim($__xsettings->pdf->fontcolordata->color[1]));
                                  $footerData['fontcolor'][2] = intval(trim($__xsettings->pdf->fontcolordata->color[2]));
                                }

                                // Wenn kein Bereich aus den Evaluations in den Body soll, darf natürlich nichts eingefügt werden
                                if (!empty($footerData)) {
                                    $ReportsFooter[] = $footerData;
                                }
                    ...
                    foreach ($_xsettings as $__xsettings) {
                        if ($__xsettings->pdf->output == 4) {
                            if (trim($tcpdf->xdata->$_Reports->{$__xsettings->key}) == '') {
                                continue;
                            }
                            ...
                            if (isset($__xsettings->pdf->showif->field)) {
                                ...
                                if (trim($tcpdf->xdata->{$field[0]}->{$field[1]}) != $value && (!isset($valueText) || trim($tcpdf->xdata->{$field[0]}->{$field[1]}) != $valueText)) {
                                    continue;
                                }
                    ...
                    if (preg_match('/Evaluation$/', $_model)) {
                        foreach ($changes as $change) {
                            ...
                            if (empty($change[0])) {
                                ...
                                if (isset($change[1]['position'])) {
                                    $txtData['data'] .= ' ' . $change[1]['position'];
                                }
                        ...
                        foreach ($changes as $field => $values) {
                            ...
                            if (!isset($tcpdf->xsettings->$_model)) {
                                continue;
                            }

                            if (!isset($tcpdf->xsettings->$_model->$field)) {
                                continue;
                            }
...
//                    $value = preg_replace('/\xc3\x9f/', "\xc3\xb8", $value);        // Ø
                        ...
                        if (isset($_ReportsHaed['measure'])) {
                            $ReportsData[$x][$_ReportsHaed['sorting']]['measure'] = trim($_ReportsHaed['measure']);
                        }
    ...
    public function WeldSorting($data)
    {
        ...
        if (count($weld_descriptions) == 0) {
            return $data;
        }

        if (count($weld_positions) == 0) {
            return $data;
        }
        ...
        if ($RotationTrue == 0) {
            foreach ($ReportsHaedSort as $_ReportsHaedSort) {
                ...
                if (
                    ...
                    $datakey = '';
                    $field = $_ReportsHaedSort['key'];
                    if (isset($tcpdf->xsettings->$ReportEvaluation->$field->headerfrom->model)) {
                        $datamodel = trim($tcpdf->xsettings->$ReportEvaluation->$field->headerfrom->model);
                    }

                    if (isset($tcpdf->xsettings->$ReportEvaluation->$field->headerfrom->key)) {
                        $datakey = trim($tcpdf->xsettings->$ReportEvaluation->$field->headerfrom->key);
                    }
                ...
                if (isset($_ReportsHaedSort['measure']) && !empty($_ReportsHaedSort['measure'])) {
                    $____data .= ' (' . $_ReportsHaedSort['measure'] . ')';
                }

                $tcpdf->SetCellPadding(1);
                $tcpdf->MultiCell(
                    floatval($_ReportsHaedSort['width']),
                    floatval($heightesCell),
                    $____data,
                    $theLineHorizontal,
                    'L',
                    false,
                ...
                $StartXCell = floatval($StartXCell) + floatval($_ReportsHaedSort['width']);
        ...
        if (isset($ReportsHaedSort)) {
            foreach ($ReportsHaedSort as $_ReportsHaedSort) {
                ...
                if (isset($_ReportsHaedSort['headline']) && $_ReportsHaedSort['headline'] == 1) {
                    ...
                    $tcpdf->MultiCell(
                        floatval($tcpdf->defs['QM_CELL_LAYOUT_CLEAR'] - $tcpdf->defs['PDF_MARGIN_LEFT']),
                        0,
                        h($_ReportsHaedSort['description']),
                        0,
                        'L',
                        1,
                        1,
...
//                $_data = setUTF8($_data);
                ...
                if (!empty($_ReportsHaedSort['break'])) {
                    $_ReportsHaedSort['break'] = 1;
                }

                if (empty($_ReportsHaedSort['width'])) {
                    $_ReportsHaedSort['width'] = $tcpdf->defs['QM_CELL_LAYOUT_WIDTH'] - $tcpdf->defs['QM_CELL_PADDING_R'];
                }

                $_ReportsHaedSort['width'] = explode(' ', $_ReportsHaedSort['width']);
                if (is_string($_ReportsHaedSort['width']) || count($_ReportsHaedSort['width']) == 1) {
                    $_ReportsHaedSort['width'] = array($_ReportsHaedSort['width'][0], $_ReportsHaedSort['width'][0]);
                }
                ...
                if (!empty($_ReportsHaedSort['description'])) {
                    ...
                    if (
                        ...
                        $tcpdf->setFont(null, 'B', $size);
                        $tcpdf->MultiCell(
                            floatval($tcpdf->defs['QM_CELL_LAYOUT_WIDTH'] - $tcpdf->defs['PDF_MARGIN_LEFT'] - 1), //$_ReportsHaedSort['width'][0],
                            0,
                            h($_ReportsHaedSort['description']),
                            0,
                            'L',
 ...
 */
                    ...
                    if (isset($_ReportsHaedSort['data_array']) && count($_ReportsHaedSort['data_array']) > 0) {
                        ...
                        $tcpdf->setFont(null, 'B', $size);
                        $tcpdf->MultiCell(
                            floatval($tcpdf->defs['QM_CELL_LAYOUT_WIDTH'] - $tcpdf->defs['PDF_MARGIN_LEFT'] - 1), //$_ReportsHaedSort['width'][1],
                            0,
                            h($_data),
                            0,
                            'L',
    ...
    {
        if (empty($width)) {
            $width = $tcpdf->defs['QM_CELL_LAYOUT_WIDTH'] - $tcpdf->defs['QM_CELL_PADDING_R'];
        }

        if (is_object($texts) || is_array($texts)) {
            $texts = reset($texts);
        }

        if (!is_string($texts) || !preg_match_all('/([^\s]+)([\s]+)*/', $texts, $matches)) {
            return;
        }
...
//            pr($childReports);
    ...
    {
        $return = '';
        if (!isset($report['Reportnumber'])) {return $return;}
        if ($format == null) {
    ...
    public function PositioningTable($tcpdf, $define, $additionsdata, $ReportPdf)
    {
        ...
        if (trim($tcpdf->xdata->$Verfahren->position_table) == 0) {
            return;
        }
    ...
    public function PrintImages($tcpdf, $define)
    {
        ...
        if ($this->request->PDFImageCount == 0) {
            return;
        }
        ...
        if ($this->request->PDFImageCount == 1) {
            $this->__PrintSingleImages($tcpdf, $define);
        } elseif ($this->request->PDFImageCount > 1) {
            $this->__PrintManyImages($tcpdf, $define);
        }
    ...
    protected function __PrintManyImages($tcpdf, $define)
    {
        ...
        switch ($this->request->PDFImageCount) {
            ...
            case 2;
                $ImagePerLine = 4;
                if (count($tcpdf->ReportImages) == 1) {
                    $ImagePerLine = 1;
                } elseif (count($tcpdf->ReportImages) == 2) {
                    $ImagePerLine = 2;
                } else {
                    $ImagePerLine = ceil(count($tcpdf->ReportImages) / 2);
                }
            ...
            case 4;
                ...
                if (count($tcpdf->ReportImages) == 2) {
                    $ImagePerLine = 2;
                }

                if (count($tcpdf->ReportImages) > 2 && $ImagePerLine > count($tcpdf->ReportImages)) {
                    $ImagePerLine = ceil(count($tcpdf->ReportImages) / 2);
                }
        ...
        if ($this->request->PDFImageCount == 2) {
            $ImagePerLine = 2;
        }
        ...
        $x_margin = 2.5;
        $text_height = 10;
        $imgcountall = count($tcpdf->ReportImages);
        $counter = 0;
        ...
        foreach ($tcpdf->ReportImages as $_key => $_ReportImages) {
            ...
            if ($Images[$_key]['imageinfo'][1] > $MaxImageHeight) {
                $MaxImageHeight = $Images[$_key]['imageinfo'][1];
            }

            //$MaxImageHeight = $define['QM_PAGE_BREAKE'] - $y_pos;
            if (!empty($_ReportImages['Reportimage']['discription'])) {
                $Images[$_key]['imageinfo']['description'] = $_ReportImages['Reportimage']['discription'];
            } else {
                $Images[$_key]['imageinfo']['description'] = ' ';
            }
        ...
        foreach ($ImageArray as $_key => $_data) {
            ...
            $imagescount = count($ImageArray);
            $i = 0;
            foreach ($_data as $_key => $__data) {
                $counter++;
                $i++;

                if ($__data['imageinfo'][1] > ($define['QM_PAGE_BREAKE'] - $y_pos)) {$__data['imageinfo'][1] = $define['QM_PAGE_BREAKE'] - $y_pos;}
                ...
                $tcpdf->MultiCell(
                    floatval($__data['imageinfo'][0]),
                    floatval($text_height),
            ...
            if ($y_pos + $MaxImageHeight > $define['QM_PAGE_BREAKE'] && $counter < $imgcountall) {
                $tcpdf->AddPage(trim($define['PDF_PAGE_ORIENTATION']), trim($define['PDF_PAGE_FORMAT']));
                $tcpdf->SetY($tcpdf->HaederEnd);
                $y_pos = $tcpdf->GetY() + $x_margin;
            }
        }
    ...
    protected function __PrintSingleImages($tcpdf, $define)
    {
        ...
        foreach ($tcpdf->ReportImages as $_ReportImages) {
            ...
            $xpos = $define['PDF_MARGIN_LEFT'] + $define['QM_CELL_PADDING_L'] + 0.5;
            $ypos = $tcpdf->GetY() + $define['QM_CELL_PADDING_T'] + 0.5;

            $border = 1;
            if ($_ReportImages['Reportimage']['border'] == 0) {
                $border = 0;
            }

            $palign = null;
            if ($_ReportImages['Reportimage']['position'] == 0) {
                $palign = null;
            }

            if ($_ReportImages['Reportimage']['position'] == 1) {
                $palign = 'C';
            }

            if ($_ReportImages['Reportimage']['position'] == 2) {
                $palign = 'R';
            }
            ...
            if ($_ReportImages['Reportimage']['size'] < 100 && $_ReportImages['Reportimage']['max_size'] == 0) {
                $imageW = $imageW * $_ReportImages['Reportimage']['size'] / 100;
                $imageh = $imageh * $_ReportImages['Reportimage']['size'] / 100;
            }

            if ($_ReportImages['Reportimage']['max_size'] == 1) {

                $max_height = $define['QM_PAGE_BREAKE'] - floor($ypos);
                $max_width = $define['QM_CELL_LAYOUT_WIDTH'] - $define['PDF_MARGIN_LEFT'];

                $new_width = $max_height * $imageW / $imageh;
                $new_height = $max_width * $imageh / $imageW;
/*
pr($max_width);
pr($max_height);
pr($imageW);
 ...
 */
            ...
            $tcpdf->Image(
                ...
                false, // resize
                $tcpdf->QM_DPI, //  dpi
                $palign, // 'L', // Ausrichtung auf Seite
                false, // Bild als Maske
                false, // Maske oder Testumfluss glaube ich
                $border, // der Rand
                true, // passt das Bild an Dimensionen bleiben
                false, // wenn das Bild nicht angezeit werden soll
                true, // Bild an Seitengrße anpassen
                false, // Alternativtext
                0// altervatives Bild
            );
    ...
    public function CreatePdfDataName($data = null, $Verfahren)
    {
        ...
        if (Configure::check('PdfNameFields') == true && !empty(Configure::read('PdfNameFields'))) {
            ...
            if(Configure::check('Separator') && !empty(Configure::read('Separator'))) $Separator = Configure::read('Separator');
            else $Separator = '_';
            ...
            foreach ($entry as $key => $value) {
                ...
                if (isset($data->$model->$field) && !empty($data->$model->$field)) {
                    ...
                    $output .= trim($data->$model->$field);
                }
            }

            $output = preg_replace('/[^a-z0-9- ]/i', '_', $output);

        }
        return $output;
    }

    public function PrintEvolutionList($tcpdf, $positions,$define){
     ...
     foreach ($positions as $p_key => $p_value) {
        ...
        foreach ($p_value as $key => $value) {
          $evaluation_key = $value['key'];
          $data = '';
          $data = $value['data'];

          //Beschreibung nur einmal ausgeben
          if($value['key'] == 'description' && $value['data'] == $descriptionlast) continue;

          $fontheigth = $tcpdf->xsettings->$Reports->$evaluation_key->pdf->fontsize;

          if($value['key'] == 'description'){
            $tcpdf->Line($define['PDF_MARGIN_LEFT'], $tcpdf->GetY(), $define['QM_CELL_LAYOUT_CLEAR'], $tcpdf->GetY(), $tcpdf->style1);
            $descriptionlast = $value['data'] ;
            $fontweight = 'B';
          }else{
            $fontweight = 'N';
          }
          ...
          if(isset($tcpdf->xsettings->$Reports->$evaluation_key->fieldtype))
          {
              $data = trim($data);
              switch(strtolower(trim($tcpdf->xsettings->$Reports->$evaluation_key->fieldtype)))
              {
                case 'checkbox':
                ...
                case 'radio':
                  ...
                  if(
                    !empty($tcpdf->xsettings->$Reports->$evaluation_key->radiooption)
                    && count($tcpdf->xsettings->$Reports->$evaluation_key->radiooption->value) > 0
                  )
                  {
                    $radiooptions = array();

                    foreach($tcpdf->xsettings->$Reports->$evaluation_key->radiooption->value as $_radiooptions){
                      array_push($radiooptions, trim($_radiooptions));
                    }
                  }

                  $data = $radiooptions[(int)$data];
                  break;
              }
          }

          $width = floatval( $tcpdf->xsettings->$Reports->$evaluation_key->pdf->positioning->width);
          $break =  $tcpdf->xsettings->$Reports->$evaluation_key->pdf->positioning->break;



          $tcpdf->SetFont($font, $fontweight, $fontheigth);
          $y = floatval($tcpdf->GetY());
