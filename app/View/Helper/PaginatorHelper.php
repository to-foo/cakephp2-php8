 ...
 */
	public function sort($key, $title = null, $options = array()) {
		...
		if(isset($this->_View->viewVars['paginateOptions']) && is_array($this->_View->viewVars['paginateOptions'])) {
			$options = array_merge($this->_View->viewVars['paginateOptions'], $options);
		}
		...
		return $this->Html->tag('p', $this->link($title, $url, $options), array('class'=>'paging'));
	}
