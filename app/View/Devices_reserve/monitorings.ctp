...
if(count($device[0]['Testingmethods']) > 0){
	foreach($device[0]['Testingmethods'] as $_key => $_data){
		if($_key > 0)echo '; ';
...
echo $this->Html->link(__('Print device infos',true),array_merge(array('action' =>'pdf'),$this->request->projectvars['VarsArray']),array('class' => 'icon icon_devices_pdf','title' => __('Print device infos',true)));
