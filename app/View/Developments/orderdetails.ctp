...
<?php echo __('Order',true) . ' ' . $deliverynumber['Order']['auftrags_nr']; ?> 
<?php echo $this->Form->create('DevelopmentData',array('class' => 'open_close_order','id' => 'open_close_order_details')); ?>
<?php
$options = array(
...
echo '<div class="input radio ordersdevelopmentsdetails">';
...
<?php echo $this->Form->end(); ?>
	...
	<tr<?php echo $class;?>>
		...
		<td class="actions">  
			<?php 
			if(isset($development['Reportnumber'])){
				echo $this->Html->link(__('Print'), array('controller' => 'reportnumbers', 'action' => 'pdf', 
				$this->request->projectvars['VarsArray'][0],
				$this->request->projectvars['VarsArray'][1],
				$this->request->projectvars['VarsArray'][2],
				$development['Reportnumber']['report_id'],
				...
				array('class'=>'icon icon_print', 'id' => 'text_delete_project','title' => __('Print this report')));
				
				echo $this->Html->link(__('Edit'), array('controller' => 'reportnumbers', 'action' => 'edit', 
				$this->request->projectvars['VarsArray'][0],
				$this->request->projectvars['VarsArray'][1],
				$this->request->projectvars['VarsArray'][2],
				$development['Reportnumber']['report_id'],
				$development['Reportnumber']['id']), 
				array('class'=>'icon icon_view ajax','title' => __('Go to report') . ' ' . $development['Reportnumber']['year'] . '-' . $development['Reportnumber']['number'])); 
			}
			echo $this->Html->link(__('Delete'), array('action' => 'progressdel', 
				$this->request->projectvars['VarsArray'][0],
				$this->request->projectvars['VarsArray'][1],
				$this->request->projectvars['VarsArray'][2],
			...
			echo $this->Html->link(__('Edit'), array('action' => 'progressedit', 
				$this->request->projectvars['VarsArray'][0],
				$this->request->projectvars['VarsArray'][1],
				$this->request->projectvars['VarsArray'][2],
				$this->request->projectvars['VarsArray'][3],
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("div.ordersdevelopmentsdetails input[type=radio]").change(function() {
				...
				$.ajax({
						type	: "POST",
						cache	: true,
/*						url		: $("#open_close_order_details").attr("action"), */
						url		: "<?php echo $this->Html->url(array('controller'=>'developments','action'=>'orderdetails',
