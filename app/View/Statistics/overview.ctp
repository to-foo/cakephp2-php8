...
//$ = $this->request->data['extra']['monts_all'];
$max = 0;
...
$dots = array();
foreach($this->request->data['extra']['welds']['statistics'][$this->request->data['extra']['diagrammdata']] as $_key => $_day){
	$day[str_replace('.','',$_key)] = $_day;
	$day[str_replace('.','',$_key)]['date'] = $_key;
	$tickLabels[str_replace('.','',$_key)] = $_key;	
}

ksort($day);
ksort($tickLabels);
/*
pr($this->request->data['extra']['welds']['statistics']['month']);
die();
*/
...
$graph->title->Set(__('Overview',true));
