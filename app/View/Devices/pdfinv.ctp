...
foreach ($Devices as $key => $DevicesData) {
  ...
  if ($newpage === true) { //1. Seite Tabellenkopf immer mit ausgegeben / wenn Neue Seite erstellt wurde -> Tabellenkopf ausgeben
    ...
    foreach ($tcpdf->xsettings->Device->children() as $_DevicesKey => $_DevicesSettings) {
      if (!isset($tcpdf->xsettings->Device->$_DevicesKey->pdf)) {
        continue;
      }
      if (empty($tcpdf->xsettings->Device->$_DevicesKey->pdf->output)) {
        continue;
      }
      $_datahead = trim($_DevicesSettings->discription->$locale);

      $tcpdf->MultiCell(
        intval($tcpdf->xsettings->Device->$_DevicesKey->pdf->positioning->width),
        $CellHeightData - 3,
        $_datahead,
      ...
      $startx = $startx + intval($tcpdf->xsettings->Device->$_DevicesKey->pdf->positioning->width);
    }
  ...
  foreach ($tcpdf->xsettings->Device->children() as $_DevicesKey => $_DevicesSettings) {
    ...
    if(isset($Devices [$key] [$_DevicesKey])) {
      $_data = trim($Devices [$key] [$_DevicesKey]) ;
    }

    // Wenn das Datenfeld leer ist, wird ein Schrägstrich gesetzt
    if (empty($_data) && Configure::read('ReplaceEmptyValuesInPrint')) {
      $_data = '/';
    }
...
$tcpdf->Line($define['PDF_MARGIN_LEFT'], $tcpdf->GetY(), intval($define['QM_CELL_LAYOUT_CLEAR']), $tcpdf->GetY()); //Letzte  hor. Linie pro Seite
