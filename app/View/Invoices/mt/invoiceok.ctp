...
echo '<h3 class="link">';
echo '<a href="javascript:" rel="toggle1" class="toggle up" title="'.__('Hide', true).'"><span>';
...
echo '<td><strong>'.__('Modification time', true).'</strong></td>';
echo '<td><strong>'.__('Editor', true).'</strong></td>';
echo '<td></td>';
//echo '<td></td>';
echo '</tr>';
...
if(isset($statisticsgenerall)){
	...
	foreach($statisticsgenerall as $_dataArray){
		...
		echo '</td>';
		echo '<td>'.@$_dataArray['Statisticsgenerall']['examinierer'].'</td>';
		echo '<td>'.@$_dataArray['Statisticsgenerall']['examination_start'].'</td>';
		echo '<td>'.@$_dataArray['Statisticsgenerall']['examination_end'].'</td>';
		echo '<td>'.@$_dataArray['Statisticsgenerall']['examination_time'].'</td>';
		...
		echo '<td>'.@$_dataArray['Statisticsgenerall']['id'].'</td>';
//		echo '<td><input type="checkbox" name="" id="" value="" checked="checked"/></td>';
		echo '</tr>';
	...
	echo '<tr class="altrow">';
		echo '<td><strong>'.__('Total', true).'</strong></td>';
		echo '<td>&nbsp;</td>';
		echo '<td>&nbsp;</td>';
		echo '<td>&nbsp;</td>'; 
...
echo '<h3 class="link">';
echo '<a href="javascript:" rel="toggle2" class="toggle up" title="'.__('Hide', true).'"><span>';
...
if(isset($statisticsstandard)){
	foreach($statisticsstandard as $key1 => $_InvoiceQuerys){
		...
		foreach($_InvoiceQuerys as $_key2 => $__InvoiceQuerys){
		
			if($_key2 == 'price_total'){
				echo '<tr>';
				echo '<td></td>';
				echo '<td></td>';
				echo '<td></td>';
				echo '<td></td>';
				echo '<td><strong>'.$__InvoiceQuerys.'</strong></td>';
				echo '<td></td>';
				echo '<td></td>';
				echo '<td></td>';
				echo '</tr>';
			...
			foreach($__InvoiceQuerys as $__key3 => $___InvoiceQuerys){
			if(isset($___InvoiceQuerys['number']) && $___InvoiceQuerys['number'] > 0){
				echo '<tr class="';
						
				if($___InvoiceQuerys['deaktiv'] == 1){
					echo 'deaktiv';
				}
				else {
					echo 'altrow';
				...
				echo '<td>'.$___InvoiceQuerys['number'].'</td>';
				echo '<td>'.$___InvoiceQuerys['dimension'].'</td>';
				echo '<td>'.$___InvoiceQuerys['position'].'</td>';
				echo '<td>'.$___InvoiceQuerys['price'].'</td>';
				echo '<td>'.$___InvoiceQuerys['price_total'].'</td>';
				echo '<td>'.$___InvoiceQuerys['modified'].'</td>';
				echo '<td>'.$___InvoiceQuerys['user_id'].'</td>';
...
echo '<h3 class="link">';
echo '<a href="javascript:" rel="toggle3" class="toggle up" title="verbergen"><span>';
echo __('Overview addiitional services', true).' ('.$statusDisc.') ';
echo '</span></a></h3>';
...
foreach($statisticsadditional as  $_key => $_statisticsadditional){
	...
	echo'<td class="i0">'.$_statisticsadditional['Statisticsadditional']['kurztext'].'</td>';
	echo'<td class="i1">'.$_statisticsadditional['Statisticsadditional']['examinierer_1'].'</td>';
	echo'<td class="i2">'.$_statisticsadditional['Statisticsadditional']['examinierer_2'].'</td>';
	echo'<td class="i3">'.$_statisticsadditional['Statisticsadditional']['date'].'</td>';
...
$("#AccountsPrint").change(function() {
	...
	var url = "<?php echo Router::url(array('controller' => 'invoices', 'action' => 'print', $this->request->projectvars['projectID'], $this->request->projectvars['equipmentType'], $this->request->projectvars['equipment'], $this->request->projectvars['orderID'], $this->request->projectvars['reportID'])); ?>/" + $(this).val();
