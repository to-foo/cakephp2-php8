...
echo '</div>';
echo $this->ViewData->ShowOrderData($orders,$settings->Order->children(),$locale); 
echo '</td></tr></table>';

echo '<h3 class="link">';
echo '<a href="javascript:" rel="toggle1" class="toggle close" title="'.__('show/Hide', true).'"><span>';
echo 'Übersicht Prüfberichte ('.$statusDisc.')</span></a>';
echo '</h3>';
echo '<table cellpadding="0" cellspacing="0" class="toggle1" id="reports_table">';
echo '<tr>';
echo '<th><strong>'.__('Report', true).'</strong></th>';
echo '<th><strong>'.__('Radiation source', true).'</strong></th>';
echo '<th><strong>'.__('No. of welds', true).'</strong></th>';
echo '<th><strong>'.__('thereof NE-welds', true).'</strong></th>';
echo '<th><strong>'.__('Films 10x24', true).'</strong></th>';
echo '<th><strong>'.__('Films 10x48', true).'</strong></th>';
echo '<th><strong>x</strong></th>';
echo '<th><strong>X</strong></th>';
echo '</tr>';
...
foreach($dataArray[$verfahren] as $_dataArray){
	...
	echo '</td>';
	echo '<td>'.@$_dataArray['radiation_source'].'</td>';
	echo '<td>'.@$_dataArray['weldnumber'].'</td>';
	echo '<td>'.@$_dataArray['weld_ne'].'</td>';
	echo '<td>'.@$_dataArray['film_10_24'].'</td>';
	echo '<td>'.@$_dataArray['film_10_48'].'</td>';
//	echo '<td class="hide"><span>'.@$_dataArray['infos']['id'].'</span></td>';
	echo '<td>';
	echo '<input title="Prüfbericht aus dieser Abrechnung ausschließen" type="checkbox" class="aktivdeaktiv" value="'.$_dataArray['infos']['id'].'" ';
	...
	if(isset($_dataArray['film_10_24'])){
		$_film_10_24_summary = $_film_10_24_summary + $_dataArray['film_10_24'];
	}
	if(isset($_dataArray['film_10_48'])){
		$_film_10_48_summary = $_film_10_48_summary + $_dataArray['film_10_48'];
	}
...
echo '<h3 class="link">';
echo '<a href="javascript:" rel="toggle2" class="toggle" title="'.__('Show/Hide', true).'">';
echo '<span>'.__('Overview Testing areas / Welds', true).' ('.$statusDisc.')</span>';
echo '</a></h3>';
...
if($WeldsTotal > $WeldsComplet){
	...
	echo '<p class="hinweis">'.__('For invocation radiation source, image no. and dimensions must be entered into report.').'</p>';
}
else {
...
if(isset($InvoiceQuerys)){
	foreach($InvoiceQuerys as $key1 => $_InvoiceQuerys){
		foreach($_InvoiceQuerys as $key2 => $__InvoiceQuerys){
			echo '<tr><td colspan="8"><strong>'.$InvoiceQuerysLabel[$key1][$key2]['label'].'</strong></td></tr>';
				echo '<tr>';
				echo '<td>'.__('Amount', true).'</td>';
				echo '<td>'.__('Dimensions', true).'</td>';
				echo '<td>'.__('Image-No.', true).'</td>';
				echo '<td>'.__('Radiation source', true).'</td>';
			...
			foreach($__InvoiceQuerys as $key3 => $___InvoiceQuerys){
				foreach($___InvoiceQuerys as $key4 => $____InvoiceQuerys){
					if(isset($____InvoiceQuerys['number']) && $____InvoiceQuerys['number'] > 0){
						...
						echo '<td class="number" headers="">';
						echo $____InvoiceQuerys['number'].'</td>';
						echo '<td class="dimension">'.$____InvoiceQuerys['infos']['dimension'].'</td>';
						echo '<td class="image_no">'.$____InvoiceQuerys['infos']['image_no'].'</td>';
						echo '<td class="radiation_source">'.$____InvoiceQuerys['infos']['radiation_source'].'</td>';
						echo '<td class="select">';
						
						if(isset($____InvoiceQuerys['selectet'])){
							$defaultArray = array('default' => $____InvoiceQuerys['selectet']);	
						}
						else {
							$defaultArray = array();	
						}
						
						if(isset($____InvoiceQuerys['infos']['saveselect'])){$selectet = $____InvoiceQuerys['infos']['saveselect'];}
						else {$selectet = null;}
						
						echo $this->Form->input($key1.'_'.$key2.'_'.$key3.'_'.$key4,array(
									'name' => 'data[Evaluation][Invoice]['.$key1.']['.$key2.']['.$key3.']['.$key4.']',
									'class' => 'createprice', 
									'label' => false, 
									'options' => $____InvoiceQuerys['select'], 
									'empty' => __('Please select', true), 
									'div' => false,
									'default' => $selectet
									)
								);
						echo $this->Form->input('number_'.$key1.'_'.$key2.'_'.$key3.'_'.$key4,array(
									'type' => 'hidden', 
									'label' => false, 
									'value' => $____InvoiceQuerys['number']
									)
								);
						echo '</td>';
						echo '<td class="singleprice"><span class="singles'.$key1.'_'.$key2.'_'.$key3.'_'.$key4.'">';
						if(isset($____InvoiceQuerys['infos']['singleprice'])){
							echo $____InvoiceQuerys['infos']['singleprice'];	
						}
						echo '</span></td>';
						echo '<td class="price"><span class="collect'.$key1.'_'.$key2.'_'.$key3.'_'.$key4.'">';
						if(isset($____InvoiceQuerys['infos']['price'])){
							echo $____InvoiceQuerys['infos']['price'];	
						}
						echo '</span></td>';
						echo '</tr>';
					}
...
echo '<table class="toggle4" cellpadding="0" cellspacing="0" id="createinvoice">';
//echo '<tr class="element altrow" id="number'.$x.'">';
...
//			echo '<table cellpadding="0" cellspacing="0">';
			...
			echo '</td>';
			echo '<td class="i5 datum" title="' . __('Edit') . '" >'.trim($__AdditionalServices->cell4).'</td>';
			echo '<td class="i6">'.trim($__AdditionalServices->cell5).'</td>';
			echo '<td class="i7 number" title="' . __('Edit') . '" >'.trim($__AdditionalServices->cell6).'</td>';
...
//			echo '</div>';
			...
			$(function() {
				...
				$("div#number'.$x.' .number").editable("'.Router::url(array_merge(array('controller' => 'invoices', 'action' => 'number'),$this->request->projectvars['VarsArray'])).'", { 
				...
				$("div#number'.$x.' .datum").editable("'.Router::url(array_merge(array('controller' => 'invoices', 'action' => 'date'),$this->request->projectvars['VarsArray'])).'", { 
					indicator : "<img src=\'img/indicator.gif\'>",
					name   : "date",
					id   : "elementid",
					select : true, 
				...
				$("#select'.$x.'").change(function() {
					...
					$.ajax({
						type	: "POST",
						cache	: false,
						url		: "'.Router::url(array_merge(array('controller' => 'invoices', 'action' => 'add'),$this->request->projectvars['VarsArray'])).'",
						data	: ({
							ajax_true: 1, 
							number: '.$x.',
							value: $(this).val()
...
$(".createprice").change(function() {
		...
		$.ajax({
			type	: "POST",
			cache	: true,
			url		: "<?php echo Router::url(array_merge(array('controller' => 'invoices', 'action' => 'invoicedata'),$this->request->projectvars['VarsArray'])); ?>",
			data	: data1,
			success: function(data) {
			$(target1).html(data);
			$(target1).show();
...
$("#make").click(function() {
	...
	data.push({name: "ajax_true", value: 1});
	data.push({name: "testingmethodID", value: <?php echo $this->request->projectvars['reportID'];?>});
	
	var StopScript = 0;
		
	$("table.toggle1 input.aktivdeaktiv").each(function(index, value) {	
		if($(this).is(':checked')){
			data.push({name: "data[Report][Invoice][]", value: $(this).val()});
		}
	});		
	...
	$("table.toggle2 tr.altrow td").each(function(index, value) {
		...
		if($(this).attr("class") == "select"){
			if($(this).children().attr("class") == "createprice" && $(this).children().val() == ''){
				...
				data.push({name: $(this).children().attr("name"), value: $(this).children().val()});
			}
		}
	});	
...
// console.log(data);
	...
	$("table.toggle4 tr.altrow td").each(function(index, value) {
		if($(this).attr("class") == "select"){
			data.push({name: $(this).children().attr("name"), value: $(this).children().val()});
		}
		if($(this).attr("class") == "number_hide"){
			data.push({name: $(this).children().attr("name"), value: $(this).children().val()});
		}
	});
...
$("#create").click(function() {
		...
		if(counter == 0){
			$('<tr class="element altrow" id="number'+counter+'"></tr>').prependTo('#createinvoice');
			insertAfterElement = "#number"+counter;
			conterforajax = counter;
			counter++;
		}
		else {
			$('<tr class="element altrow" id="number'+counter+'"></tr>').insertAfter('#'+$(".element").last().attr('id'));
			insertAfterElement = "#number"+counter;
			conterforajax = counter;
			counter++;
		}
