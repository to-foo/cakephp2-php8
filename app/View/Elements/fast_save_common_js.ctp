...
<?php
if(isset($this->request->projectvars['VarsArray'])) $url = array_merge(array('action'=>'save'), $this->request->projectvars['VarsArray']);
else $url = array('action' => 'save'); 
...
$(document).ready(function(){
	...
	$("form.dialogform input, form.dialogform select, form.dialogform textarea").each(function(){
		...
		if($(this).closest("div").hasClass("text") || $(this).closest("div").hasClass("textarea")) {
			orginal_data["orginal_" + $(this).attr("name")] = $(this).val();
		}
		if($(this).closest("div").hasClass("select")) {
			if($(this).is('select')) {
				orginal_data["orginal_" + $(this).attr("name")] = $(this).find(':selected').attr('value');
			} else {
				orginal_data["orginal_" + $(this).attr("name")] = $(this).val();
			}
		}
