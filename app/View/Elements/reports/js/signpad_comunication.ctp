...
$signtransport = $this->html->url(array_merge(array('action' => 'mass_report_action'),$this->request->projectvars['VarsArray']));
?>
</div>
...
//			get_signature_auto();
		...
		$("input.get_signature").click(function() {
			if($(this).hasClass("open_pad")){
				getSignature();
			}
			if($(this).hasClass("close_pad")){
				$("#Status_pad_0 span").html("<?php echo __('The current connection has been closed, please try again',true);?>");
				$("#Status_pad_0").removeClass("success");
				$("#Status_pad_0").addClass("fail");
				close_pad();
			}
			changeSigButton("#SignBtn");
			return false;
		});


            function changeSigButton(data) {

				if($(data).hasClass("open_pad")){
					$(data).val("<?php echo __('close sign pad',true);?>");
					$(data).removeClass("open_pad");
					$(data).addClass("close_pad");
				}
				else if($(data).hasClass("close_pad")){
					$(data).val("<?php echo __('open sign pad',true);?>");
					$(data).removeClass("close_pad");
					$(data).addClass("open_pad");
				}
			}
            ...
            if (window.WebSocket === undefined) {
				$(state + " span").text("sockets not supported");
...
//                state.innerHTML = "Connected to server " + wsUri;
 				$(state + " span").text("<?php echo __('Connected to WSS service',true);?>");
...
//                state.innerHTML = "Communication error";
 				$(state + " span").text("Communication error");
				$(state).removeClass("success");
				$(state).addClass("fail");
            }
...
//						var ctx = $(sigcanva).getContext('2d');
                        ctx.fillStyle = $("#showPaletteOnly").val();
                        ctx.strokeStyle = $("#showPaletteOnly").val();
                        ctx.lineWidth = 1.5;
                        ctx.lineCap = "round";
                  ...
                  if ((obj.TOKEN_CMD_ORIGIN == "TOKEN_CMD_SIGNATURE_CONFIRM") || (obj.TOKEN_CMD_ORIGIN == "TOKEN_CMD_API_SIGNATURE_CONFIRM")) {
                       ...
                       var ret = obj.TOKEN_PARAM_RETURN_CODE;
						...
						var data = $("ReportnumberMassReportActionForm").serializeArray();
						data.push({name: "ajax_true", value: 1});
						data.push({name: "dialog", value: 1});
						data.push({name: "signature", value: obj.TOKEN_PARAM_SIGNATURE_SIGN_DATA});
...
/*
console.log('TOKEN_CMD_SIGNATURE_IMAGE');
console.log(obj);
console.log('------------------------------');
*/
                        //check the return code
						...
						data.push({name: "ajax_true", value: 1});
						data.push({name: "dialog", value: 1});
						data.push({name: "image", value: obj.TOKEN_PARAM_FILE});
...
//                            alert("The search for pads failed. Reason: " + obj.TOKEN_PARAM_ERROR_DESCRIPTION);
                        ...
                        if (obj.TOKEN_PARAM_CONNECTED_PADS == null) {
							changeSigButton("#SignBtn");
							$("#Status_pad_0 span").html("<?php echo __('No connected pads have been found. Make sure that a device is connected.',true);?>");
							$("#Status_pad_0").removeClass("success");
							$("#Status_pad_0").addClass("fail");
                            searchState = searchStates.setPadType;
                            openState = openStates.openPad;
						...
						$("#PadType_0 span").html(obj.TOKEN_PARAM_CONNECTED_PADS[0].TOKEN_PARAM_PAD_TYPE);
						$("#SerialNumber_0 span").html(obj.TOKEN_PARAM_CONNECTED_PADS[0].TOKEN_PARAM_PAD_SERIAL_NUMBER);
						$("#FirmwareVersion_0 span").html(obj.TOKEN_PARAM_CONNECTED_PADS[0].TOKEN_PARAM_PAD_FIRMWARE_VERSION);
						$("#Status_pad_0 span").html("<?php echo __('Sign pad conected');?>");
						$("#Status_pad_0").removeClass("fail");
						$("#Status_pad_0").addClass("success");
...
//                            alert("Failed to close pad. Reason: " + obj.TOKEN_PARAM_ERROR_DESCRIPTION);
                        ...
                        //check for connected pads
                        if (ret == 0) {
                            alert("No connected pads have been found. Make sure that a device is connected.");
							//search finished, reset search state
                            searchState = searchStates.setPadType;
                            openState = openStates.openPad;
                            preparationState = preparationStates.setBackgroundTarget;
						...
						//print device info
						$("#PadType_0 span").html(obj.TOKEN_PARAM_TYPE);
						$("#SerialNumber_0 span").html(obj.TOKEN_PARAM_SERIAL);
						$("#Status_pad_0 span").html("<?php echo __('Sign pad conected');?>");
						$("#Status_pad_0").removeClass("fail");
						$("#Status_pad_0").addClass("success");
...
//                        document.getElementById("SerialNumber_0").innerHTML = obj.TOKEN_PARAM_SERIAL;
						...
						$("#FirmwareVersion_0 span").html(obj.TOKEN_PARAM_VERSION);
             //           document.getElementById("FirmwareVersion_0").innerHTML = obj.TOKEN_PARAM_VERSION;
                        ...
                        document.getElementById("SignData_0").value = null;
//                        document.getElementById("SignData_0").value = obj.TOKEN_PARAM_SIGN_DATA;
            ...
            */
            ...
            function get_signature_auto() {

				message = '{"TOKEN_TYPE": "TOKEN_TYPE_REQUEST", "TOKEN_CMD": "TOKEN_CMD_SIGNATURE_START", "TOKEN_PARAM_FIELD_NAME": "Customer Signature", "TOKEN_PARAM_CUSTOM_TEXT": "Please sign here!"}';
            ...
            function getSignature() {
				...
				$("#PadType_0 span").html(null);
				$("#SerialNumber_0 span").html(null);
				$("#FirmwareVersion_0 span").html(null);
				$("#Status_pad_0 span").html(null);
				$("#Status_pad_0").removeClass("success");
            ...
            function send_signatory() {
				message = '{"TOKEN_TYPE": "TOKEN_TYPE_REQUEST", "TOKEN_CMD": "TOKEN_CMD_SIGNATURE_RETRY"}';
				websocket.send(message);
				logMessage(message);
            }

            function open_pad_autostart() {
...
//console.log(obj);
            ...
            function signature_start() {
				...
				if (websocketMode == websocketModes.Default) {
					// default mode
					message = '{ "TOKEN_TYPE":"TOKEN_TYPE_REQUEST", "TOKEN_CMD":"TOKEN_CMD_SIGNATURE_START", "TOKEN_PARAM_FIELD_NAME":"Signature Examiner", "TOKEN_PARAM_CUSTOM_TEXT":"Please sign!" }';
				}
 				else {
					alert("invalid websocketMode");
					return;
...
//                alert("The pad has been disconnected.");
				changeSigButton("#SignBtn");
				$("#Status_pad_0 span").html("<?php echo __('The pad has been disconnected.');?>");
				$("#Status_pad_0").removeClass("success");
				$("#Status_pad_0").addClass("fail");
            }
