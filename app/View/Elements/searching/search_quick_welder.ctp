...
$FormName = 'QuicksearchWelder';
$FormId = 'QuicksearchWelderForm';

$ResultParms = array_fill(0, 15, 0);
...
echo $this->Form->input('json_url', array(
				...
				'label' => false,
				'div' => false,
				'value' => $this->Html->url(array_merge(array('controller' => 'welders','action' => $action),$this->request->projectvars['VarsArray'])),
			)
		);
...
echo $this->Form->input('target_action', array(
		...
		echo $this->Form->input('target_result', array(
						...
						'label' => false,
						'div' => false,
						'value' => $this->Html->url(array_merge(array('controller' => 'welders','action' => $targedaction),$ResultParms)),
					)
				);
