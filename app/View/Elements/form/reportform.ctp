...
foreach ($setting as $_setting) {
  ...
  if (!empty($_setting->title)) {
    $attribut_array['title'] = trim($_setting->title->$lang);
  }
  ...
  //$attribut_array = $this->ViewForm->EditOnlyBy($data,$_setting,$setting,$attribut_array,$hidable,$disabled,$x);
  $attribut_array = $this->ViewForm->EditAfterClosing($data,$_setting,$setting,$attribut_array,$hidable,$disabled,$x);
