...
foreach ($setting as $_setting) {
    ...
    if (isset($data['Reportnumber']['status']) && $data['Reportnumber']['status'] != 0) {
        ...
        if (isset($data['Reportnumber']['revision_write']) && $data['Reportnumber']['revision_write'] == 1) {
            ...
            if (Configure::check('NotAllowRevision') && $data['Reportnumber']['status'] >= Configure::read('NotAllowRevision')) {
                ...
                if (isset($_setting->allowrevision) && trim($_setting->allowrevision) > 0) {
                    $disabled = false;
                }
    ...
    if ($_setting->fieldset != '' && $_setting->fieldset == 1) {
        ...
        if (isset($_setting->multiselect) && trim($_setting->key) != 'error') {
            $fieldset_class .= 'multiple_field';
        }

        if (isset($_setting->multiselect) && trim($_setting->key) == 'error') {
            $fieldset_class .= 'multiple_field_error';
        }
    ...
    if (trim($_setting->format) == 'date') {
        echo $this->ViewForm->CreateDateInputForReport($data, $_setting, $setting, $attribut_array, $hidable, $disabled, $x);
    }
