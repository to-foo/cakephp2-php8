...
$(document).ready(function(){
	...
	function ReplaceInput(data) {
		...
		if(typeof(data.Dropdown) == "undefined") return false;
		...
		$.each(data.Dropdown,function(key,value) {
			...
			if(element.is("input") === true){
				...
				let newElement = '<select name="' + name + '" class="hasDependencies" id="' + item + '"></select>';
				let label = $(item).closest("div").find("label");

				$(newElement).insertAfter(item);
				$("input" + item).remove();
			...
			success: function(data) {
				...
				if(typeof(data.Dropdown) != "undefined") {
					...
					$.each(data.Dropdown,function(key,value) {
						...
						if(value.sortoptions){
							...
							$.each(value.sortoptions, function(index, option){

								if(oldtext == option.value){
				...
				if(typeof(data.Depentency) != "undefined") {
					...
					$.each(data.Depentency,function(key,value) {
						...
						if($(item).is("select") === true) oldvalue = $("#"+currentid+" option:selected").text();
						...
						if(value.save != undefined && value.save == "direct" && step == "update"){
							...
							if(value.edit_after_closing && value.edit_after_closing == 1){
								clss += " edit_after_closing";
							}

							hiddeninput = '<textarea id="'+ key +'" disabled="disabled" class="' + clss + '" name="'+ value.name +'">'+insert+'</textarea>';
						...
						if(data.Reportnumber.status > 0 && data.Reportnumber.revision_progress < 1) disabled = "disabled='disabled'";
						else disabled = '';
						...
						if(step == "initial"){
							...
							if(value.length > 0){
								...
								$.each(value.options, function(index, val){
									if(val == oldvalue) value_exists =  true;
					    	});
								if(value_exists === false){
									$("#" + currentid).append("<option value='"+oldvalue+"'>"+oldvalue+"</option>");
									$("#" + currentid + " option[value='"+oldvalue+"']").attr("selected","selected");
								}
							}
						}
	...
	$('select.hasDependencies,input.hasDependencies').each(function(){
