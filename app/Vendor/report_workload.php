...
class XTCPDF extends TCPDF
{
	...
	function Header() {
		...
		if($this->xsettings->{$this->ReportPdf}->settings->QM_FIRM_ADRESS_FORMAT->SHOW_ADRESS == 1){
			...
			if($this->xdata->Testingcomp->id != 0){
...
//						$this->xsettings->$ReportPdf->settings->QM_FIRM_ADRESS_FORMAT->POS_Y + 1,
		...
		$headerSections = array(
			'initiator' => array(
				'fontsize' => 7,
				'content' => 'KW Niederaußem'
			)
		);

		// Logo Betreiber
		if(file_exists(Configure::read('company_logo_folder').DS.'1'.DS.'operator.png')){
		...
		{
			$this->reportnumber = trim($this->xdata->Report->identification) . " / ";
			$this->reportnumber .= trim($this->xdata->Reportnumber->number) . " / ";
			$this->reportnumber .= trim($this->xdata->Reportnumber->year);
		}
