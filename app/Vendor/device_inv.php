...
//*Überschrift
	...
	if(trim($this->xsettings->$DevicePdf->settings->QM_HEADLINE_01->VALUE) != ''){
				...
				$this->MultiCell(
								0,
								0,
								trim($this->xsettings->$DevicePdf->settings->QM_HEADLINE_01->VALUE).' '. (!empty($this->xdata['testingmethod']) ? $this->xdata['testingmethod'] ['verfahren'] : 'Alle'),
								0,
								'L',
								0,
								1,
								intval($this->xsettings->$DevicePdf->settings->QM_HEADLINE_01->POS_X),
								intval($this->xsettings->$DevicePdf->settings->QM_FIRM_ADRESS_FORMAT->POS_Y),
								true,
                                                                0,
								false,
								true,
...
// zweite Überschrift z.B. englisch
			if(trim($this->xsettings->$DevicePdf->settings->QM_HEADLINE_02->VALUE) != ''){
				$this->SetFont('calibri', 'n', 10);
				$this->MultiCell(
								0,
								0,
								trim($this->xsettings->$DevicePdf->settings->QM_HEADLINE_02->VALUE),
								0,
								'L',
								0,
								1,
								intval($this->xsettings->$DevicePdf->settings->QM_HEADLINE_02->POS_X),
								$this->GetY(),
								true,
									0,
								false,
	...
	intval($this->xsettings->$DevicePdf->settings->QM_LOGO_X),
	intval($this->xsettings->$DevicePdf->settings->QM_FIRM_ADRESS_FORMAT->POS_Y),
	intval($this->xsettings->$DevicePdf->settings->QM_LOGO_HEIGHT),
	intval($this->xsettings->$DevicePdf->settings->QM_LOGO_WIDTH),
	'PNG', false, '', false, 300, '', false, false, 0, false, false, false);
        }
        $this->Ln();

        			if($this->xsettings->$DevicePdf->settings->QM_FIRM_ADRESS_FORMAT->SHOW_ADRESS == 1){
				$Firmenadresse  = null;
				...
				$this->MultiCell(
								...
								0,
								1,
								intval($this->xsettings->$DevicePdf->settings->QM_FIRM_ADRESS_FORMAT->POS_X),
                                                                intval($this->xsettings->$DevicePdf->settings->QM_FIRM_ADRESS_FORMAT->POS_Y),
								true,
									0,
								false,
