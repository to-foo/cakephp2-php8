...
class XTCPDF extends TCPDF
{
	...
	function Header() {
		...
		if($this->xsettings->{$this->ReportPdf}->settings->QM_FIRM_ADRESS_FORMAT->SHOW_ADRESS == 1){
			$Firmenadresse  = null;

			foreach($this->xsettings->{$this->ReportPdf}->settings->QM_FIRM_ADRESS_FORMAT->items as $_items){
			...
			if(true || $this->xdata->Testingcomp->id != 0){
/*
...
/**/
				// Logo Prüfunternehmen
				if(file_exists(Configure::read('company_logo_folder').'1'.DS.'logo.png') && $this->xdata->Testingcomp->id != 1){
					$this->Image(Configure::read('company_logo_folder').'1'.DS.'logo.png',
					$this->xsettings->$ReportPdf->settings->QM_LOGO_X + 1,
					$this->xsettings->$ReportPdf->settings->QM_FIRM_ADRESS_FORMAT->POS_Y + 0.5, //$this->xsettings->$ReportPdf->settings->QM_FIRM_ADRESS_FORMAT->POS_Y + 1,
					$this->xsettings->$ReportPdf->settings->QM_LOGO_WIDTH,
					$this->xsettings->$ReportPdf->settings->QM_LOGO_HEIGHT,
					'PNG', false, '', false, '', '', false, false, 0, 'LT', false, false);
				}

				$headerSections['initiator'] = array(
					'offset' => array(
						'x' => $this->xsettings->$ReportPdf->settings->QM_LOGO_WIDTH + 3
					),
					'content' => $Firmenadresse,
					'fontsize' => 7,
					//'content' => 'Wacker Chemie AG'
				);
			} else {
				$username =
					$this->xuser['name']. "\n".
...
/**/
		...
		// Logo Betreiber (ausgeblendet)
		if(false && file_exists(Configure::read('company_logo_folder').DS.'1'.DS.'operator.png')){
			$size = getimagesize(Configure::read('company_logo_folder').DS.'1'.DS.'operator.png');

			$this->Image(Configure::read('company_logo_folder').DS.'1'.DS.'operator.png',
			$this->defs['PDF_MARGIN_LEFT'] + 2,
			$this->defs['PDF_MARGIN_TOP'] + 3,
			40,
			0,
			'PNG', false, '', false, '', '', false, false, 0, false, false, false);
		}
...
//			$PrueferWacker = __("examiner no. Wacker", true);
		...
		array_push($headerSections, array(
			...
			'valign'=>'T',
			'break'=>0,
			'content'=> CakeSession::check('statistics.params.Reportnumber.welding_company') ? __('Welding company: %s', CakeSession::read('statistics.params.Reportnumber.welding_company')) : null
			//'fontsize'=> 8
		));
...
//			$section['content'] = setUTF8($section['content']);
