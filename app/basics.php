...
if(!function_exists('setUTF8')) {
	function setUTF8($data) {
		while(mb_detect_encoding(utf8_decode($data)) != 'ASCII') {
			$data = utf8_decode(ConnectionManager::getDataSource('default')->setUTF8Encoding($data));
		}
...
/**
 * recursive array map
 * @param function $callback function to evaluate on each element of $array
 * @param array $array (multidimensional) Array to be mapped
 * @return array New array with mapped elements
 */
	function array_rmap($callback, $array) {
		return array_map(function($elem) use($callback) {
			if(is_array($elem)) return array_rmap($callback, $elem);
			else return call_user_func($callback, $elem);
		}, $array);
	}
}

if (!function_exists('preg_grep_key')) {
/**
 * filters arraykeys by pattern
 *
 * @param string $pattern Regex pattern
 * @param array $array Array to filter
 * @param options int $flags set to PREG_GREP_INVERT returns elements with keys NOT matching the pattern
 * @return array Array of elements with keys matching $pattern
 */

	function preg_grep_key($pattern='/.*/', $array = array(), $flags = 0) {
		// flags -> PREG_GREP_INVERT invertes pattern
